...
TEST(M1S06EitherInteropFromOther, U01TaskAwaitsEitherSuccess)
{
...
TEST(M1S06EitherInteropFromOther, U02TaskAwaitsEitherError)
{
...
TEST(M1S06EitherInteropFromOther, U03TaskAwaitsMultipleEithers)
{
  ...
  auto task = []() -> SimpleTask<int>
  {
    ...
    EXPECT_EQ(e1.state(), ropic::CoroState::DONE);
    EXPECT_EQ(e2.state(), ropic::CoroState::DONE);
    EXPECT_EQ(e3.state(), ropic::CoroState::DONE);
...
TEST(M1S06EitherInteropFromOther, U04TaskAwaitsEitherMixedResults)
{
  ...
  auto task = []() -> SimpleTask<int>
  {
    ...
    EXPECT_EQ(success.state(), ropic::CoroState::DONE);
    EXPECT_EQ(error.state(), ropic::CoroState::DONE);
...
TEST(M1S06EitherInteropFromOther, U05TaskAwaitsEitherVoid)
{
  ...
  auto task = []() -> SimpleTask<bool>
  {
    ...
    EXPECT_EQ(success.state(), ropic::CoroState::DONE);
    EXPECT_EQ(failure.state(), ropic::CoroState::DONE);
...
TEST(M1S06EitherInteropFromOther, U06TaskAwaitsNestedEither)
{
  RecordProperty("id", "M1-S06-U06");
  RecordProperty("ver", "0.02");
  RecordProperty(
      "desc",
      "Task coroutine co_awaits Either from nested Either "
      "coroutine");

  auto task = []() -> SimpleTask<int>
  {
    auto result = co_await level1(0);
    EXPECT_EQ(result.state(), ropic::CoroState::DONE);
...
TEST(M1S06EitherInteropFromOther, U07TaskAwaitsEitherLvalue)
{
  ...
  auto task = []() -> SimpleTask<int>
  {
    ...
    EXPECT_EQ(ref.state(), ropic::CoroState::DONE);
